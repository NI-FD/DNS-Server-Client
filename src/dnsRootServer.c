...
int main(int argc, char *argv[]){
    ...
    serverAddr.sin_addr.s_addr = inet_addr("127.2.2.1");
//    serverAddr.sin_addr.s_addr = htonl(INADDR_ANY);
    ...
    while(1){
        ...
        //查询数据库是否存在
        if(checkFile(query_packet.question->name, query_packet.question->qType) == 1){
            //不存在，告诉下一个服务器
            if (query_packet.question->qType == 1 || query_packet.question->qType == 5) {
                ...
                answer_packet.header->tag = standard_res_AA_NRA;
                getPFile(query_packet.question->name, A, answer_packet.authority);
                ...
                answer_MX_packet.header->tag = standard_res_AA_NRA;
                answer_MX_packet.header->authorityNum = 1;
                getPFile(query_packet.question->name, A, answer_MX_packet.authority);
        ...
        else if(checkFile(query_packet.question->name, query_packet.question->qType) == 0){
...
//            answer_packet.authority->data_len = strlen("127.2.2.1");
//            answer_packet.authority->Data = "127.2.2.1";
            printPacket(answer_packet);
...
int checkFile(char *name, uint16_t type){
    ...
    FILE *fp;
    fp = fopen("rootconfig.txt", "r");//假定存在这个文件中。
    if(fp == NULL)
    {
        printf("ERROR!\n");
        return -1;
    ...
    while(~fscanf(fp,"%s%s%s%d%s", fname, ftype, fclass, &fttl, frdata)){
        if(strcmp(fname, cutDomain(nname,1)) == 0){
            printf("In check\n");
            printf("fname:%s\n",fname);
            printf("ftype:%s\n",ftype);
            printf("fclass:%s\n",fclass);
...
int getPFile(char *name, uint16_t type, struct dns_rr *rr){
    ...
    char *filename = (char *)malloc(BUFER_SIZE);
    memset(filename, 0, BUFER_SIZE);
    //    char fttl[BUFER_SIZE];
    int fttl;
    char frdata[BUFER_SIZE];
    int datalen = 0;
