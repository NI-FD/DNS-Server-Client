...
int main(int argc, char *argv[]){
    ...
    serverAddr.sin_addr.s_addr = inet_addr("127.3.3.1");
...
//            answer_packet.authority->data_len = strlen("127.3.3.1");
//            answer_packet.authority->Data = "127.3.3.1";
            printPacket(answer_packet);
...
int checkFile(char *name, uint16_t type){
    ...
    FILE *fp;
    fp = fopen("tldcaconfig.txt", "r");//假定存在这个文件中。
    if(fp == NULL)
    {
        printf("ERROR!\n");
        return -1;
